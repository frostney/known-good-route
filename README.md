...
| [`run-retro`](run-retro/SKILL.md) | Review a completed workstream from conversation, repository, and forge evidence; use the required `grilling` skill to agree durable lessons; then directly update the project's vision and readiness/completion definitions after explicit confirmation. |
...
- `run-retro` requires `grilling` to conduct the retrospective interview and
  final confirmation; it never substitutes an ad-hoc question loop.
