...
async function reference(name: string, file: string): Promise<string> {
  return Bun.file(resolve(repositoryRoot, name, "references", file)).text();
}
...
describe("cross-skill policy", () => {
  test("implementation options share evidence and validation before selection", async () => {
    const source =
      (await skill("implement")) +
      (await reference("implement", "approach-selection.md"));
  ...
  test("implementation comparisons and retrospectives work in conversation", async () => {
    const source = await skill("implement");
    const approach = await reference("implement", "approach-selection.md");
    const retro = await skill("run-retro");

    expect(source).toMatch(/Present the\s+comparison directly in conversation/);
    expect(approach).toContain("require the registered `grilling` skill");
    expect(approach).toMatch(/pros, cons,\s+scores, uncertainties/);
    expect(retro).toContain("Present the\nretrospective in the conversation");
    expect(retro).toMatch(
      /file edits and other mutations require the selection/,
    );
  });
  ...
  test("milestone telemetry stays normalized, one-shot, and provider neutral", async () => {
    ...
    expect(source).toContain(
      "event-ledger `validate` and `summarize` commands",
    );
  ...
  test("GitHub prose uses exact attributed Notes", async () => {
    ...
    const addressPrFeedback = await skill("address-feedback");
    ...
    expect(addressPrFeedback).toContain("concise, complete");
  });

  test("PR feedback repeats code review and black-box testing before the project gate", async () => {
    const source = await reference("address-feedback", "pr.md");
  ...
  test("stack feedback freezes reviewed heads and admits only the complete stack", async () => {
    const source =
      (await skill("address-feedback")) +
      (await reference("address-feedback", "stack.md"));
    const readiness = await Bun.file(
      resolve(repositoryRoot, "address-feedback/references/stack-readiness.md"),
    ).text();

    expect(source).toContain("one repository-scoped native stack number");
    expect(source).toContain(
      "Review every initial member once for its exact head",
    );
    expect(source).toContain(
      "create exactly one new branch above the current top",
    );
    expect(source).toContain("never push those fixes into any frozen member");
    expect(source).toContain(
      "never merge, merge-queue entry, automatic merge, or purchased review capacity",
    );
    expect(source).toContain(
      "An explicit read-only request disables every mutation",
    );
    expect(readiness).toContain(
      "A covered lower member can still contain the reported flaw",
    );
  ...
  test("stack feedback keeps CodeRabbit behavior in executable adapter code", async () => {
    const source = await reference("address-feedback", "stack.md");
    const adapter = await Bun.file(
      resolve(repositoryRoot, "address-feedback/scripts/coderabbit_adapter.py"),
    ).text();

    expect(source).toContain("Provider-neutral behavior is the default");
    expect(source).toMatch(
      /does not\s+own stack identity, findings, fixes, readiness/,
    );
  ...
  test("milestone rush delegates a native stack once and retains merge authority", async () => {
    ...
    expect(source).toContain(
      "`/address-feedback` once with the verified stack identity",
    );
  ...
  test("implementation owns the review and behavior-testing loop", async () => {
    ...
    expect(source).toContain("same unchanged implementation");
    expect(source).toMatch(
      /If fixing a gate\s+failure changes the implementation, return to step/,
    );
  });

  // Continuation and worker ownership are exercised through native history
  // cases and worker-evidence tests; exact prose is not an acceptance contract.
  ...
  test("writing guidance uses revision thresholds and conditional detail", async () => {
    ...
    expect(source).toContain("Revise where it helps");
    expect(source).toContain("application-generated reports");
    expect(source).toContain(
      "Reserve `byte-identical` for compiler or binary output",
    );
  ...
  test("writing guidance replaces compressed labels with specific wording", async () => {
    ...
    expect(source).toContain(
      "Coined labels that compress a multi-step process",
    );
    ...
    expect(source).toContain(
      "Preserve quoted headings, fixture keys, and code identifiers",
    );
  });
  ...
  test("code review keeps nitpicks canonical but non-blocking", async () => {
    ...
    expect(findings).toContain("BLOCKING | IMPORTANT | IMPROVEMENT | NITPICK");
  });
  ...
  test("Pascal preserves standard initialisms", async () => {
    const source = await Bun.file(
      resolve(
        repositoryRoot,
        "native-nostalgia-stack/references/code-style.md",
      ),
    ).text();
